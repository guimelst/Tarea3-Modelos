...
sigmax=np.sqrt((((df['x']**2)*df['p']).sum())-(mux**2))#encontramos sigma para x
print ('\u03C3X = ',sigmax)

sigmay=np.sqrt((((df['y']**2)*df['p']).sum())-(muy**2))#encontramos sigma para y
print ('\u03C3Y =',sigmay)
...
plt.plot(ax,gx)
plt.savefig('Fundión de densidad marginal X')
plt.plot(ay,gy)
plt.savefig('Fundión de densidad marginal Y')
...
def f(ax,ay):
  return (0.02683* np.exp(-0.0636981*((ax-9.93536)**2)-0.0177*ay**2))

ax = np.linspace(5,15,21)
ay = np.linspace(5,25,21)
gx = f(ax,ay)

axe.plot3D( ax, ay, gx, 'red')
axe.scatter3D(ax, ay, gx, )
plt.savefig('Fundión de densidad conjuta')
